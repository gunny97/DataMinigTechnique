...
def calculate_word_similarity_across_sentences(
	...
	for word in tqdm(word2sent_indexer):
...
if __name__ == "__main__":
	...
	# for _, folder_name in [ko_sbert_multitask, msbert, kosimcse, koroberta, kcelectra]:
	for _, folder_name in [ko_sbert_multitask, msbert, kosimcse, koroberta, kcelectra]:

		word2sent_indexer = json.load(open(f'{folder_name}/word2sent.json', 'r'))

		EMBEDDINGS_FULL_PATH = os.path.join(EMBEDDINGS_PATH, f'{folder_name}/{folder_name}.hdf5')
		...
		calculate_word_similarity_across_sentences(EMBEDDINGS_FULL_PATH, word2sent_indexer, 
			f'{folder_name}/self_similarity.csv')
		...
		variance_explained_by_pc(EMBEDDINGS_FULL_PATH, word2sent_indexer,
			f'{folder_name}/variance_explained.csv', os.path.join(EMBEDDINGS_PATH, f'pcs/{folder_name}.pc.'))

		print(f"Exploring embedding space ...")
		explore_embedding_space(EMBEDDINGS_FULL_PATH, f'{folder_name}/embedding_space_stats.json')
